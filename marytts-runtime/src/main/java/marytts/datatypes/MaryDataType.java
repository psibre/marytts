...
public class MaryDataType
{
    ...
    /**
     * Provide an example text for this data type, for the given locale,
     * if one is available.
     ...
     * When the registration is not (yet) complete, calls to 
     * @see #registerModule(MaryModule, Locale, Voice) are possible;
     * when the registration is complete, calls to the other methods are possible.
     *  
     ...
     * @throws IllegalStateException if called when registration was already completed before.
     */
    public static void setRegistrationComplete()
     ...
     * @throws IllegalStateException if this method is called while registration is ongoing.
     */
    ...
    /**
     * Provide a list of known data types, i.e. of data types used by
     * any of the known modules, partially sorted in the order of processing. 
     ...
     * @param type
     * @param locale
     * @return an example text suitable for type and locale, or null if
     * none is available.
     */
    public static String getExampleText(MaryDataType type, Locale locale)
    {
        ...
        if (locale == null) {
            ...
            exampleStream = type.getClass().getResourceAsStream("/marytts/language/"+locale.toString()+"/datatypes/"+type.name()+"."+locale.toString()+".example");
        }
